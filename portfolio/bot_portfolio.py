...
# from django.utils import timezone
#
# from portfolio.models import AccountHistory
# from trading.models import Trade, Instrument
#
#
# class PortfolioBot:
...
#         trades = Trade.objects.filter(profile=self.profile, is_open=True)
#         total_equity = self.profile.equity()
#         asset_summary = {}
#         unrealized_pl = Decimal('0.00')
#
#         for trade in trades:
#             asset = trade.instrument.name
#             current_price = trade.instrument.current_price()
#             current_value = trade.volume * current_price
#             trade_unrealized_pl = (current_price - trade.entry_price) * trade.volume
#
...
#             asset_summary[asset]['unrealized_pl'] += trade_unrealized_pl
#             asset_summary[asset]['volume'] += trade.volume
#             unrealized_pl += trade_unrealized_pl
#
#         # Calculate allocation percentages
#         for asset, data in asset_summary.items():
#             allocation = (data['total_value'] / total_equity) * 100
#             asset_summary[asset]['allocation'] = allocation
#
#         return asset_summary, unrealized_pl, total_equity
#
#     def rebalance_portfolio(self):
#         asset_summary, unrealized_pl, total_equity = self.calculate_portfolio_state()
#
#         # Example logic to rebalance (This should be based on a real strategy)
#         for asset, data in asset_summary.items():
#             if data['allocation'] > 50:  # Example threshold
#                 print(f"Rebalancing {asset}, reducing exposure...")
#                 self.execute_trade(asset, volume_to_sell=Decimal(1))  # Sell logic (needs actual implementation)
#             elif data['allocation'] < 10:  # Example to buy
#                 print(f"Rebalancing {asset}, increasing exposure...")
#                 self.execute_trade(asset, volume_to_buy=Decimal(1))  # Buy logic (needs actual implementation)
#
#         # After rebalancing, update account history
#         self.update_account_history(asset_summary, unrealized_pl, total_equity)
#
#     def execute_trade(self, asset, volume_to_sell=None, volume_to_buy=None):
#         instrument = Instrument.objects.get(name=asset)
#         current_price = instrument.current_price()
#
#         if volume_to_sell:
#             print(f"Selling {volume_to_sell} of {asset}")
#             # Logic to sell the asset
#             self.create_trade(instrument, current_price, volume_to_sell, Trade.SELL)
#
#         if volume_to_buy:
#             print(f"Buying {volume_to_buy} of {asset}")
#             # Logic to buy the asset
#             self.create_trade(instrument, current_price, volume_to_buy, Trade.BUY)
#
#     def create_trade(self, instrument, price, volume, trade_type):
#         Trade.objects.create(
#             profile=self.profile,
#             instrument=instrument,
#             currency_pair=instrument.name,
#             volume=volume,
#             entry_price=price,
#             trade_type=trade_type,
#             open_date=timezone.now(),
...
#         )
#
#     def update_account_history(self, asset_summary, unrealized_pl, total_equity):
#         AccountHistory.objects.create(
#             profile=self.profile,
#             timestamp=timezone.now(),
...
#             realized_pl=self.profile.realized_pl(),
#             asset_allocation=asset_summary  # Store the current allocation
#         )
#
